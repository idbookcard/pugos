...
Route::prefix('packages')->name('packages.')->group(function () {
    ...
    Route::get('/custom-order', [PackageController::class, 'customOrder'])->name('third-party');
...
// 后台路由
Route::prefix('master')->name('master.')->middleware(['auth', \App\Http\Middleware\MasterMiddleware::class])->group(function () { 
    // 后台首页
    
     // 仪表盘
     Route::get('/', [DashboardController::class, 'index'])->name('master.dashboard');
    ...
    Route::prefix('packages')->name('packages.')->group(function () {
        ...
        // 额外选项同步路由
        Route::post('/{id}/sync-extras', [AdminPackageController::class, 'syncExtras'])->name('sync-extras');
        Route::post('/batch-sync-extras', [AdminPackageController::class, 'batchSyncExtras'])->name('batch-sync-extras');
    });

    Route::prefix('categories')->name('categories.')->group(function () {
        Route::get('/', [AdminPackageCategoryController::class, 'index'])->name('index');
        Route::get('/create', [AdminPackageCategoryController::class, 'create'])->name('create');
        Route::post('/store', [AdminPackageCategoryController::class, 'store'])->name('store');
        Route::get('/{category}/edit', [AdminPackageCategoryController::class, 'edit'])->name('edit');
        Route::put('/{category}', [AdminPackageCategoryController::class, 'update'])->name('update');
        Route::delete('/{category}', [AdminPackageCategoryController::class, 'destroy'])->name('destroy');
    });
    ...
    Route::prefix('orders')->name('orders.')->group(function () {
        ...
        // 工单管理路由
Route::post('/{order}/weekly-tasks/{weekNumber}/work-order', [OrderController::class, 'createWorkOrder'])
->name('master.orders.create-work-order');
Route::put('/{order}/weekly-tasks/{weekNumber}/work-order', [OrderController::class, 'updateWorkOrder'])
->name('master.orders.update-work-order');
    });
    ...
    Route::prefix('extras')->name('extras.')->group(function () {
        Route::get('/', [App\Http\Controllers\Admin\ExtraOptionController::class, 'index'])->name('index');
        Route::get('/create', [App\Http\Controllers\Admin\ExtraOptionController::class, 'create'])->name('create');
        Route::post('/', [App\Http\Controllers\Admin\ExtraOptionController::class, 'store'])->name('store');
        Route::get('/{id}/edit', [App\Http\Controllers\Admin\ExtraOptionController::class, 'edit'])->name('edit');
        Route::put('/{id}', [App\Http\Controllers\Admin\ExtraOptionController::class, 'update'])->name('update');
        Route::delete('/{id}', [App\Http\Controllers\Admin\ExtraOptionController::class, 'destroy'])->name('destroy');
        
        // 同步与导入
        Route::get('/sync', [App\Http\Controllers\Admin\ExtraOptionController::class, 'sync'])->name('sync');
        Route::get('/import', [App\Http\Controllers\Admin\ExtraOptionController::class, 'showImport'])->name('import.form');
        Route::post('/import', [App\Http\Controllers\Admin\ExtraOptionController::class, 'import'])->name('import');
        
        // API路由
        Route::get('/api/list', [App\Http\Controllers\Admin\ExtraOptionController::class, 'apiList'])->name('api.list');
        Route::post('/status/update', [App\Http\Controllers\Admin\ExtraOptionController::class, 'updateStatus'])->name('status.update');
        Route::post('/prices/update', [App\Http\Controllers\Admin\ExtraOptionController::class, 'updatePrices'])->name('prices.update');
        Route::post('/{id}/translate', [App\Http\Controllers\Admin\ExtraOptionController::class, 'translate'])->name('translate');
    });
    ...
    Route::prefix('api-settings')->name('api-settings.')->group(function () {
        ...
        Route::get('/{id}/test', [ApiSettingController::class, 'testConnection'])->name('test');
