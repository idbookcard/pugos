...
<script>
    ...
    if (hasEmptyFields) {
        e.preventDefault();
        console.error('表单验证失败，以下字段为空:', invalidFields);
        alert('请填写所有必填字段');
        return false;
    }
    
    // 在表单提交前记录一下所有将要提交的字段值
        ...
        try {
            ...
            if (priceMatch) {
                ...
                if (!isNaN(price)) {
                    ...
                    console.log('添加复选框价格:', price, '总额外价格:', this.extrasPrice);
                }
            }
        } catch (e) {
    ...
    if (selectedRadio && selectedRadio.value) {
        try {
            ...
            if (priceMatch) {
                ...
                if (!isNaN(price)) {
                    this.extrasPrice += price;
                    console.log('添加单选价格:', price, '总额外价格:', this.extrasPrice);
                }
            }
        } catch (e) {
    ...
    // 计算总价 (基础价格 * 数量 + 额外选项)
    // 注意：通常额外选项应该跟数量挂钩，但根据您的业务逻辑可能有所不同
    this.totalPrice = (base * this.quantity) + this.extrasPrice;
    console.log('最终总价:', this.totalPrice);
...
checkBalance() {
    ...
    if (submitBtn) {
        ...
        console.log('用户余额:', userBalance, '订单总价:', this.totalPrice);
        
        if (this.totalPrice > userBalance) {
    ...
    setTimeout(() => {
        if (window.Alpine) {
            // 找到所有 Alpine 组件并调用更新价格方法
            document.querySelectorAll('[x-data]').forEach(el => {
                if (el.__x && typeof el.__x.$data.updateTotalPrice === 'function') {
                    el.__x.$data.updateTotalPrice();
                    console.log('初始化时更新价格');
                }
            });
        }
    }, 200);
    ...
    document.querySelectorAll('input[type="checkbox"][name^="extras"], input[type="radio"][name="extras_selection"]').forEach(input => {
        input.addEventListener('change', function() {
            console.log('选项变更:', this.name, this.checked || this.value);
            // 触发 Alpine 组件中的方法
            if (window.Alpine) {
                const form = document.getElementById('orderForm').__x.$data;
                if (typeof form.updateTotalPrice === 'function') {
    ...
    // 确保总余额的计算和显示正确
    @auth
    console.log('用户总余额:', {{ auth()->user()->balance }});
    @endauth
});
